...
export interface ComputePermissionParams {
  userId: string;
  guildOwnerId: string;
  everyoneRolePermissions: bigint;
  userRolePermissions: bigint[];
  userRoleIds: string[];
 ...
 */
export function computePermissions(params: ComputePermissionParams): bigint {
  const {
    userId,
    guildOwnerId,
...
export function hasPermission(computedPerms: bigint, requiredPerm: bigint): boolean {
  return (
    (computedPerms & Permissions.ADMINISTRATOR) === Permissions.ADMINISTRATOR ||
